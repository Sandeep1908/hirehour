    ...
    const verifySubscriptionId = async () => {
      try {
        const respose = await axiosrecruiterinstance.get(
          `/api/recruiter/owner-recruiter/subscription-callback/${subscriptionId}`,
        );
  ...
  return (
    <div className="w-full min-h-[calc(100vh-56px)] flex justify-center items-center">
      <div className="flex justify-center flex-col items-center mt-[-100px]">
        <div
          className={`relative w-36 h-36 rounded-full  ${checkStatus ? 'bg-green-500' : ' bg-red-500'}  p-7 flex justify-center items-center `}
        >
          <FaRegCreditCard className="text-white w-full h-full" />
          <div
            className={`absolute ${checkStatus ? 'bg-green-500' : ' bg-red-500'} p-1 rounded-full bottom-[25px] left-[15px]`}
          >
            {checkStatus ? (
        ...
        {checkStatus ? (
          <>
            ...
            <p className="text-base text-black pt-1 text-center font-medium">
        ...
        <p className="text-base text-gray-500 pt-6 text-center">
          You will be redirected to the {checkStatus ? 'dashboard' : 'pricing'} shortly {countdown}s
        </p>
      </div>
    </div>
  );
