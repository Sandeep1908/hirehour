...
const Signin: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (userCredentials: UserCredentials) => {
      const response = await axiosInstance.post("/api/recruiter/login", userCredentials);
    ...
    onSuccess: (data) => {
      ...
      localStorage.setItem('topequatorTokenRecruiter',data?.token)
      ...
      if (decodedToken && decodedToken.permissions) {
        ...
        if (permissions.length === 1) {
           navigate('/dashboard-rtr'); // Redirect to Dashboard
        } else if (permissions.length > 1) {
           navigate('/job-poster'); // Redirect to RTR-Dashboard
        }
     }
    ...
    onError: (error) => {
      ...
      localStorage.removeItem("topequatorTokenRecruiter");
  ...
  return (
    ...
    <div className='w-full h-full px-5 lg:px-10 flex gap-20'>
     ...
     <Link to={"/job-poster/signup"} className='text-[14px] md:text-[16px] font-semibold mt-12 md:mt-4 text-center'>
