...
const AddExperience: React.FC<AddExperiences> = ({ setAddExperiencePopup }) => {
  ...
  const getRoleIdByName = (name: string) =>
   jobRole?.jobRoles?.find((role: { id: number; roleName: string }) => role.roleName === name)?.id ||
    'N/A';



    const getDomainIdByName = (name: string) =>
      domains?.domains?.find((domain: { id: number; domainName: string }) => domain.domainName === name)
        ?.id || 'N/A';
  ...
  const handleSubmit = (e: React.FormEvent,roleName:string,domainName:string) => {
    e.preventDefault();

    const formData = {
      companydetails,
      role:getRoleIdByName(roleName),
      durationStart,
      place,
      jobCategory,
      descriptionOfExperience,
      domain:getDomainIdByName(domainName),
      jobType,
    };
    ...
    <div className="w-full h-full flex p-3   justify-center items-center  fixed inset-0 transition-all duration-500 opacity-1 scale-[1.01] z-[40] ">
      ...
      <div className="max-w-[700px] h-full w-full bg-white rounded-lg overflow-y-auto z-[30] p-4">
          ...
          <div className="bg-[#F2F2F5] rounded-lg p-4">
            <div>
              ...
              <div className="w-full flex gap-4 mt-3 ">
                <div className="w-full ">
                  ...
                  <input
                    ...
                    className="w-full h-[40px] rounded-md border-[1px] border-[#E1E1E2] mt-2 px-4 "
                  />
                </div>
              </div>
              <div className="w-full flex gap-4 mt-3">
              ...
              <div className="w-full flex gap-4 mt-3">
                <div className="w-full ">
                  <p className="text-sm font-medium">
                    Job Summary <span className="text-red-600">*</span>
                  </p>
                  <textarea
                    name=""
                    id=""
                    value={descriptionOfExperience}
                    onChange={(e) => setDescriptionOfExperience(e.target.value)}
                    className="w-full h-[190px] rounded-md border-[1px] border-[#E1E1E2] mt-2 p-4 "
                  ></textarea>
                </div>
              </div>
            </div>
        ...
        <div className="w-full flex justify-end  gap-10  items-center">
          <div
            onClick={(e)=>handleSubmit(e,role,domain)}
