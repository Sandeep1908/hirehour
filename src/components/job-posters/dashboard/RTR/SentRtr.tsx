...
const SentRTR: React.FC = () => {
  const { data: sentRTR, isPending } = useQuery({
    queryKey: ['sentrtr'],
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col space-y-4">
        {isPending && (
          <div className="flex justify-center  space-x-5 items-center w-full h-[60vh]">
            <p>Loading </p>
            <Spinner size={30} color="#104B53" loading={isPending} />
          </div>
        )}

        {sentRTR?.data?.lenght !== 0 ? (
          ...
          <div className="h-[60vh] w-full flex flex-col justify-center items-center max-w-5xl space-y-5">
            <img src={NoRTRImg} className="w-56 h-48" alt="verify-job" />
            <h1 className="text-sm font-semibold text-center">No RTR Available</h1>

            <p className="text-xs text-center">
              Start sending RTR that will show in the Right to represent
            </p>
          </div>
        )}
      </div>
